...
import { acquisitionCost, apport, grossAnnualRent } from "./property";
...
/** Indicative ceilings for non-blocking UI warnings (tax amounts unchanged). */
export const MICRO_FONCIER_CEILING = 15_000;
export const MICRO_BIC_CEILING = 77_700;
export const DEFICIT_FONCIER_GLOBAL_CEILING = 10_700;

export function detectMicroCeilingWarning(property: Property): string | null {
	const rent = grossAnnualRent(property);
	if (property.regime === "IR_MICRO" && rent > MICRO_FONCIER_CEILING) {
		return `Loyer brut annuel (${Math.round(rent)} €) au-delà du plafond indicatif micro-foncier (15 000 €). Calcul conservé en micro — vérifiez le régime réel.`;
	}
	if (property.regime === "LMNP_MICRO" && rent > MICRO_BIC_CEILING) {
		return `Recettes annuelles (${Math.round(rent)} €) au-delà du plafond indicatif micro-BIC (77 700 €). Calcul conservé en micro — vérifiez le régime réel.`;
	}
	return null;
}

/**
 * Warning when simulated annual deficit imputation on global income would exceed
 * the indicative 10 700 € ceiling. Does not change annualTax amounts.
 */
export function detectDeficitImputationWarning(
	priorDeficit: number,
	netBeforeDeficit: number,
): string | null {
	if (netBeforeDeficit <= 0) return null;
	const imputation = Math.min(priorDeficit, netBeforeDeficit);
	if (imputation > DEFICIT_FONCIER_GLOBAL_CEILING) {
		return `Imputation de déficit foncier simulée (${Math.round(imputation)} €) au-delà du plafond indicatif 10 700 €/an sur le revenu global. Le modèle ne plafonne pas l'impôt — indicative.`;
	}
	return null;
}
...
export function annualTax(input: AnnualTaxInput): AnnualTaxResult {
	...
	const resultBeforeDeficit =
		grossRent - deductibleCharges - loanInterest - loanInsurance - amortization;
