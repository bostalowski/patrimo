...
export type ProjectionOptions = {
	...
	/**
	 * Optional workbook-level paliers (**Assurance emprunt**). When provided,
	 * rows for this property override `modeAssurance` (D4). When omitted,
	 * falls back to `property.assurancePaliers` (already scoped).
	 */
	paliers?: LoanInsurancePalier[];
	/** Optional workbook-level taxe foncière history (per property × year). */
...
export function annualIrr(cashFlows: number[]): number | null {
	...
	// Stryker disable all: Newton / bisection arithmetic is covered by public IRR
	// oracle tests; mutating every +/−/×/÷ in the solver is not product-signal.
...
export function projectProperty(
	...
	const modeAssurance = property.modeAssurance ?? "CRD";
	const assuranceMensuelle = property.assuranceMensuelle ?? 0;
	const insurancePaliers =
		options.paliers !== undefined
			? filterPaliersForProperty(
					normalizeLoanInsurancePaliers(options.paliers),
					property.id,
				)
			: (property.assurancePaliers ?? []);
	...
	const balanceNow = remainingBalance(
		loan,
		Math.min(monthsElapsedLoan, loan.durationMonths),
	);
	const currentMonthlyInsurance = monthlyInsuranceForLoan({
		remainingBalance: balanceNow,
		monthsElapsed: monthsElapsedLoan,
	...
	for (let k = 1; k <= horizon; k += 1) {
		...
		for (let m = monthsStart + 1; m <= monthsEnd; m += 1) {
			...
			loanInsurance += monthlyInsuranceForLoan({
				remainingBalance: balanceStart,
				monthsElapsed: m - 1,
				principal: loan.principal,
				annualInsuranceRate: loan.annualInsuranceRate,
				modeAssurance,
...
export const REAL_ESTATE_ASSUMPTIONS_FR =
	...
	"loyers et charges non récup. indexés au taux de revalorisation du bien " +
	"(pas l’IRL légal), sauf indexation à 0 % ; taxe foncière résolue par année " +
	"calendaire (historique lorsqu’il est renseigné, sinon montant du bien — non " +
	"indexée avec le loyer) ; assurance emprunteur selon le mode du bien " +
	"(CRD, capital initial, ou forfait €/mois) — ou le calendrier de paliers de la feuille " +
	"**Assurance emprunt** lorsqu’il est renseigné (les paliers priment sur le mode) ; " +
...
/** Short FR label for the active borrower-insurance rule on a property. */
export function loanInsuranceRuleLabelFr(property: {
	modeAssurance?: string;
	assurancePaliers?: { anneeDebut: number; assuranceMensuelle: number }[];
}): string {
	if ((property.assurancePaliers?.length ?? 0) > 0) {
		return "calendrier de paliers (feuille Assurance emprunt)";
	}
	switch (property.modeAssurance) {
		case "CAPITAL_INITIAL":
			return "taux sur capital initial (prime plate)";
		case "MONTANT_FIXE":
			return "forfait €/mois";
		case "CRD":
		default:
			return "taux sur capital restant dû (CRD)";
	}
}
