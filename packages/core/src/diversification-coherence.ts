...
function contributionToKey(
	...
	geoRows: GeographicAllocation[] | undefined,
	sectorRows: SectorAllocation[] | undefined,
): number {
	...
	if (isSectorKey(key)) {
		if (!sectorRows || sectorRows.length === 0) return 0;
		const sum = sectorRows.reduce((total, row) => total + row.weight, 0);
		if (!isValidGeographicWeightSum(sum)) return 0;
		return lookThroughSectorWeights(sectorRows)
			.filter((row) => row.sector === key)
			.reduce((total, row) => total + amount * row.weight, 0);
	}

	if (!geoRows || geoRows.length === 0) return 0;

	const sum = geoRows.reduce((total, row) => total + row.weight, 0);
	if (!isValidGeographicWeightSum(sum)) return 0;

	let granularity: ReturnType<typeof geographicAllocationGranularity>;
	try {
		granularity = geographicAllocationGranularity(geoRows.map((row) => row.country));
	} catch {
		return 0;
	}
	...
	if (isIsoCountryKey(key)) {
		...
		return lookThroughCountryWeights(geoRows)
		...
		const stockNumerator = liquidPositions.reduce(
			(total, pos) =>
				...
				contributionToKey(
					...
					geoByAsset.get(pos.assetId),
					sectorByAsset.get(pos.assetId),
				),
			0,
		);
