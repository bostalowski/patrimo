...
export function RealEstateProjection({
	...
	const series: ScenarioSeries[] = [
		...
		{ key: "equity", label: "Patrimoine net", color: "#10b981" },
		{
			key: "equity_real",
			label: "Patrimoine net (euros réels)",
			color: "#10b981",
			dashed: true,
		},
	];
	...
	return (
		<div className="space-y-6">
			<Card>
				<CardBody className="pt-6">
					...
					<p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
						Ce taux revalorise la valeur du bien et indexe loyers/charges (pas
						l&apos;IRL). Laisser vide = valeur workbook.
					</p>
			...
			<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
				<Card>
					<CardHeader>
						<CardTitle>Patrimoine net à {horizon} ans</CardTitle>
				...
				<Card>
					<CardHeader>
						<CardTitle>Gain total sur l&apos;horizon</CardTitle>
						<CardValue className={signClass(projection.totalReturn)}>
							{formatEuro(projection.totalReturn)}
						</CardValue>
						<p className="text-xs text-zinc-500">
							Cash-flows + revente nette − apport (pas un gain annuel)
						</p>
						{projection.cagr != null && (
							<p className="text-xs text-zinc-500">
								CAGR (apport → net si revente) :{" "}
								{formatPercent(projection.cagr)}
							</p>
						)}
						{projection.irr != null && (
							<p className="text-xs text-zinc-500">
								TRI (flux annuels) : {formatPercent(projection.irr)}
							</p>
						)}
						{projection.apport === 0 && (
							<p className="text-xs text-zinc-500">
								CAGR / TRI non calculables (apport nul)
							</p>
						)}
					</CardHeader>
				</Card>
			...
			{projection.warnings.length > 0 && (
				<ul className="space-y-1 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-300">
			...
			<Card>
				<CardHeader>
					<CardTitle>Valeur du bien et patrimoine net</CardTitle>
					<p className="text-xs leading-relaxed text-zinc-500">
						Valeur revalorisée (violet) et patrimoine net (vert) ; en pointillé,
						les mêmes après inflation ({formatPercent(inflation.rate)}).
						L&apos;apport est en gris.
					</p>
				</CardHeader>
				<CardBody>
			...
			<Card>
				...
				<CardBody className="px-0">
					...
					<p className="px-6 pt-4 text-xs leading-relaxed text-zinc-400">
						{REAL_ESTATE_ASSUMPTIONS_FR} Pour ce bien : assurance ={" "}
						{loanInsuranceRuleLabelFr(selected)}.
						{isLmnpReel
							? " En LMNP réel, les amortissements déduits sont réintégrés dans la plus-value de revente (réforme 2025)."
							: ""}{" "}
						Un seul champ « Revalo annuelle » pilote à la fois la valeur du bien
						et l&apos;indexation des loyers/charges (sauf indexation forcée à 0
						%).
					</p>
				</CardBody>
			</Card>
