...
const OVERFLOW_ENVELOPE_OPTIONS = Object.keys(ENVELOPE_LABELS) as Envelope[];
...
function formatOverflowMessage(event: EnvelopeOverflowEvent): string {
	const years = Math.max(1, Math.ceil(event.firstOverflowMonth / 12));
	const yearLabel = `année ${years}`;
	if (event.monthlySurplus != null && event.monthlySurplus > 0) {
		return `${event.source} plafond atteint en ${yearLabel} → ${formatEuro(event.monthlySurplus)}/mois redirigés vers ${event.target}`;
	}
	return `${event.source} plafond atteint en ${yearLabel} → surplus redirigé vers ${event.target}`;
}
...
export function EnvelopeProjection({
	...
	const heldByEnvelope = useMemo(() => {
		const map = new Map(envelopes.map((e) => [e.envelope, e]));
		return map;
	}, [envelopes]);

	const overflowProjection = useMemo(() => {
		const specs = envelopes.map((envelope) => ({
		...
		return projectEnvelopesWithOverflow({
			envelopes: specs,
			years: horizonYears,
	...
	const projections = overflowProjection.projections;
	const overflowEvents = overflowProjection.overflows;
	...
	const projectRealCapacity = useCallback(
		(years: number) => {
			...
			const engaged = rows.reduce(
				(sum, row) => sum + row.result.finalRealValue,
				0,
			);
	...
	const advice = useMemo(() => {
		const rows = projections.filter((p) => {
			if (heldByEnvelope.has(p.envelope)) return true;
			return p.result.totalContributed > 0;
		});
		if (rows.length === 0) return [];
		return buildEnvelopeProjectionAdvice({
			horizonYears,
			envelopes: rows.map((p) => {
				const held = heldByEnvelope.get(p.envelope);
				return {
					envelope: p.envelope,
					openDate: held?.openDate ? new Date(held.openDate) : undefined,
					plafond: held?.plafond,
					grossGain: p.result.gain,
					totalContributed: p.result.totalContributed,
				};
			}),
		});
	}, [projections, horizonYears, heldByEnvelope]);
	...
	return (
		<div className="space-y-6">
			<Card>
				<CardBody className="pt-6">
					<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
						...
						<Field label="Surplus plafond redirigé vers">
							<select
								value={overflowEnvelope}
								onChange={(e) =>
									setOverflowEnvelope(e.target.value as Envelope)
								}
								className={inputClasses}
							>
								{OVERFLOW_ENVELOPE_OPTIONS.map((envelope) => (
					...
					<p className="mt-4 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
						...
						ci-dessus (un seul saut). Le PER se simule à part avec sa fiscalité
						propre. Capitalisation mensuelle.
					</p>
				</CardBody>
			</Card>
			...
			<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
				...
				<Card>
					<CardHeader>
						...
						<CardValue className={signClass(projectedNominal - currentTotal)}>
			...
			{overflowEvents.length > 0 ? (
				<div className="space-y-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-100">
					{overflowEvents.map((event) => (
						<p
							key={`${event.source}-${event.target}-${event.firstOverflowMonth}`}
						>
							{formatOverflowMessage(event)}
						</p>
					))}
				</div>
			) : null}
			...
			<Card>
				<CardHeader>
					...
					<p className="text-xs leading-relaxed text-zinc-500">
						Projection à l&apos;horizon de tes investissements déjà engagés
						(DCA), avec le gain net après la fiscalité propre à chaque
						enveloppe.
					</p>
				</CardHeader>
				<CardBody className="px-0">
									...
									<TR key={row.envelope}>
										...
										<TD>
											{plafondValue ? (
												<Badge
													variant={
														plafondMonth !== null ? "warning" : "default"
													}
												>
													{plafondMonth !== null
					...
					<p className="px-6 pt-4 text-xs leading-relaxed text-zinc-400">
						Hypothèse PFU 30 % (12,8 % IR + 17,2 % PS). L&apos;option pour le
						barème progressif et les abattements d&apos;assurance-vie ne sont
						pas chiffrés. Pour le PER, la valeur nette intègre l&apos;économie
						d&apos;impôt à l&apos;entrée et l&apos;IR sur le capital à la sortie
						(détail ci-dessous). Conseils indicatifs, ne constituent pas un
			...
			{perActive ? (
				<Card>
					...
					<CardBody className="px-0">
							...
							<TBody>
								...
								<TR>
									<TD>
										...
										<p className="text-xs text-zinc-500">
											À effort d&apos;épargne identique : le PER investit le
											brut, le CTO le net après impôt.
										</p>
									</TD>
						...
						<p className="px-6 pt-4 text-xs leading-relaxed text-zinc-400">
							{formatEuro(perVersementsFuturs)} versés ×{" "}
							{formatPercent(tmiNowValue)} d&apos;économie d&apos;impôt.
							L&apos;avantage vs CTO compare à effort d&apos;épargne identique :
							le CTO n&apos;investit que le net (après impôt sur le revenu),
							faute de déduction. Hypothèses : versements supposés intégralement
							déductibles (plafond non vérifié), sortie en capital, IR de sortie
							approché par le TMI à la retraite. La sortie en rente et le barème
							progressif exact ne sont pas modélisés.
						</p>
					</CardBody>
				</Card>
			) : null}
