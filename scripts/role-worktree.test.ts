...
describe("scripts/role-worktree.sh", () => {
  ...
  it("N10: prints the Framer prompt read verbatim from cadrage-lock.md (not duplicated in the script)", () => {
    fx = createFixture("feat/role-framer");
    ...
    const res = fx.run("scripts/role-worktree.sh", ["framer"], { FEATURE_FLOW_WORKTREE_DIR: wtDir });
  ...
  it("N10: prints the Checker prompt read verbatim from scoring-rubric.md", () => {
    fx = createFixture("feat/role-checker-prompt");
    wtDir = mkdtempSync(path.join(tmpdir(), "role-worktree-wt-"));
  ...
  it("E2: refuses on main/master", () => {
    ...
    const res = fx.run("scripts/role-worktree.sh", ["checker"]);
    expect(res.status).not.toBe(0);
  });

  it("E8: creates a plain detached git worktree (no external tool dependency)", () => {
    fx = createFixture("feat/role-e8");
    wtDir = path.join(tmpdir(), `role-worktree-e8-${Date.now()}-${Math.random().toString(36).slice(2)}`);

    const res = fx.run("scripts/role-worktree.sh", ["checker"], { FEATURE_FLOW_WORKTREE_DIR: wtDir });

    expect(res.status).toBe(0);
    expect(res.stdout).toContain("Created worktree at");
    expect(res.stdout).toContain(wtDir);
    const branch = execFileSync("git", ["-C", wtDir, "rev-parse", "--abbrev-ref", "HEAD"]).toString().trim();
    expect(branch).toBe("HEAD"); // detached — safe to coexist with the live branch checkout
  });

  it("E7: --publish fails when the worktree touched a file other than that branch's PROGRESS.md", () => {
    fx = createFixture("feat/role-e7-fail");
    fx.writeProgress("# Progress\n\n## Notes\n");
    fx.commitAll("init progress");
    wtDir = path.join(tmpdir(), `role-worktree-e7fail-${Date.now()}`);
    execFileSync("git", ["-C", fx.root, "worktree", "add", "--detach", wtDir, "feat/role-e7-fail"]);
    execFileSync("mkdir", ["-p", path.join(wtDir, "src")]);
    writeFileSync(path.join(wtDir, "src", "not-allowed.ts"), "export const x = 1;\n");

    const res = fx.run("scripts/role-worktree.sh", ["checker", "--publish", wtDir]);
    expect(res.status).not.toBe(0);
  ...
  it("E7: --publish succeeds and copies PROGRESS.md when only that file changed", () => {
    fx = createFixture("feat/role-e7-pass");
    fx.writeProgress("# Progress\n\n## Notes\n");
    fx.commitAll("init progress");
    wtDir = path.join(tmpdir(), `role-worktree-e7pass-${Date.now()}`);
    execFileSync("git", ["-C", fx.root, "worktree", "add", "--detach", wtDir, "feat/role-e7-pass"]);
