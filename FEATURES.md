...
| Real estate | done | partial | Mobile read-only; projection/snapshot KPIs from `@patrimo/core` (ADR 0026) |
| Fiscalité | done | partial | Mobile realized only |
| Fees | done | partial | |
| Projection | done | partial | Web: envelope overflow at plafond → fallback (ADR 0016); immobilier KPIs share core with web (ADR 0026); envelope overflow mobile deferred |
