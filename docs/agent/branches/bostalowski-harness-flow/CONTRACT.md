...
| D7 — Prompt duplication | Where role prompts live | LOCKED | Single source of truth: Framer/Challenger/teach-back prompts stay in `cadrage-lock.md`; Checker prompt stays in `scoring-rubric.md`. `orca-role.sh` reads/prints them (N10), never re-types them. | Duplicate prompts into a new `orca-role.sh`-adjacent doc for convenience (rejected — exactly the "cognitive debt" duplication risk the branch itself targets) |
...
- [x] Files / packages expected to change:
  ...
  - Scripts: `scripts/{red-evidence,test-guard,gauntlet,pr-check,pr,flow-status,orca-role}.sh`, `scripts/lib/diff.sh`, extend `scripts/branch-ready.sh`
