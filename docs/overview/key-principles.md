...
Transactions, assets, accounts, budget lines, properties, DCA plans, manual prices, geographic allocations, diversification targets, and financial goals live in a `.xlsx` workbook. Local JSON and AsyncStorage automatic price files are derived caches, not authoritative portfolio state.
