...
Measure of alignment between saved **Diversification target** bands, current liquid stock, and annualized DCA flow mix. Computed by `assessDiversificationCoherence`. Geo bands use look-through from `Exposition geo` on **all** asset types (including `CRYPTO`). The `CRYPTO` band uses `AssetType.CRYPTO` at full value. Geo and crypto axes **overlap by design** — the same euro may count toward both a country/region band and the `CRYPTO` band. Statuses: `aligned`, `misaligned` (`band_drift` and/or `flow_misalign`). Returns null (card hidden) when no bands are saved or liquid invested is zero. The Diversification menu breakdown (`aggregatePortfolioDiversificationBreakdown`) uses full liquid MV: geo slices plus unmapped geo sum to 100 %; crypto is a separate slice on the same denominator. Account and asset geo charts still use covered market value ([ADR 0010](../adr/0010-partial-geographic-allocation-weights.md)). Never uses HHI / Top1 / Top3 ([ADR 0006](../adr/0006-portfolio-risk-readability.md)).
