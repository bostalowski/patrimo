...
Measure of alignment between saved **Diversification target** bands, current liquid stock (look-through geo + crypto by `AssetType`), and annualized DCA flow mix. Computed by `assessDiversificationCoherence`. Statuses: `aligned`, `misaligned` (`band_drift` and/or `flow_misalign`). Returns null (card hidden) when no bands are saved or liquid invested is zero. The Diversification menu breakdown (`aggregatePortfolioDiversificationBreakdown`) and band percentages use full liquid MV / full annual DCA. Account and asset geo charts still use covered market value ([ADR 0010](../adr/0010-partial-geographic-allocation-weights.md)). Never uses HHI / Top1 / Top3 ([ADR 0006](../adr/0006-portfolio-risk-readability.md)).
