...
2. **Required sheets** — A valid workbook contains at least `Transactions`, `Actifs`, and `Comptes`. `Budget`, `Immobilier`, and `DCA` are optional and created when first needed.
