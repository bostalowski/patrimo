...
2. **Required sheets** — A valid workbook contains at least `Transactions`, `Actifs`, and `Comptes`. `Budget`, `Immobilier`, `DCA`, `Prix manuels`, `Exposition geo`, and `Cibles diversification` are optional and created when first needed. Legacy `Allocation cible` is ignored on read and removed on write.
