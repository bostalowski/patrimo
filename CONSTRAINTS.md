...
19. MUST NOT declare a task done without green verification for the change set:
    - `verify-static` (always): `make verify` (lint + typecheck + unit tests).
    - `verify-behavior` (when behavior changes): targeted tests for the touched package/path.
    - `verify-e2e` (when web UI, API routes under `src/app/api`, workbook I/O, or settings paths change): `make e2e` / `make verify-full`.
...
22. MUST NOT treat the implementing agent's self-assessment as completion proof. For non-trivial product changes, a **separate agent** (subagent / Task tool / fresh empty-context session — not a same-session role switch) MUST score against the branch contract and verification commands before declaring done. The checker worktree is a write sandbox only; it does not by itself make the grade independent. See [docs/howto/maker-checker.md](docs/howto/maker-checker.md), [ADR 0030](docs/adr/0030-checker-agent-isolation.md).
...
24. When `verify-behavior` applies (behavior change; CONTRACT `verify-behavior` is not `n/a`), MUST NOT write production code for a CONTRACT behavior case until a targeted test for that case has been: (1) written, (2) actually run, and (3) shown failing for the **missing behavior** (not a typo, bad import, or compile noise). Record RED evidence in branch PROGRESS (or a run log). Then implement the minimal code that turns that test green. Procedure: [docs/howto/tdd-red-green.md](docs/howto/tdd-red-green.md). MUST NOT treat Spec-Driven Development (SPEC LOCK / Diátaxis package) as required — that remains opt-in.
25. When `verify-behavior` applies (Tier B cadrage), MUST NOT start Maker implementation until branch CONTRACT Intent, behavior cases, and product decisions are filled with no `OPEN` decisions, human teach-back is accepted (recorded in branch PROGRESS), Challenger Pass is recorded when the CONTRACT says `Challenger: required`, and `make branch-ready` exits 0. Procedure: [docs/howto/cadrage-lock.md](docs/howto/cadrage-lock.md). On Tier B, the Framer (cadrage) MUST NOT be the sole judge that cadrage is complete when Challenger is required — use a distinct Challenger pass. MUST NOT invent behavior absent from CONTRACT behavior cases.
26. A CONTRACT with many behavior cases MUST ship as small, separately-reviewable tranches — one row of its `## Tranches` table each — rather than one large undifferentiated diff. A tranche MAY be its own PR (stacked, waiting for merge before the next tranche's commits) or a distinct commit/commit-group reviewed incrementally within one open PR; the CONTRACT's D1-equivalent decision picks the mechanic, since separate PRs on a shared branch require a merge before the next tranche can push without growing the prior PR. Every nominal/edge behavior case MUST be assigned to at least one Tranches row (`make branch-ready` enforces this on Tier B). Diff size beyond a configured threshold MUST surface as an informational CI signal; it MUST NOT block merge on its own. Procedure: [docs/howto/feature-flow.md](docs/howto/feature-flow.md).
27. On a diff touching `@patrimo/core`, workbook I/O, or `src/app/api` routes, `make gauntlet` MUST pass before `make pr-check`: no test file/block deleted or `.skip`/`.only` added without a `Test-removal-justified:` line in branch PROGRESS, and no surviving mutant above the configured threshold in the diff's changed `packages/core/src/**` files. The Checker (`make checker`) MUST run as a **separate agent** (subagent / Task / fresh empty-context session) whose working directory is a worktree separate from the Maker's, and MUST NOT write anything besides that branch's `PROGRESS.md`. Maker-session self-check after `make checker` is a harness violation even when that worktree exists. Procedure: [docs/howto/feature-flow.md](docs/howto/feature-flow.md), [ADR 0030](docs/adr/0030-checker-agent-isolation.md).
