...
} from "@patrimo/core/realestate/projection";
...
import { colors, shared, type Theme, useThemeColors } from "../lib/theme";
...
export default function ProjectionScreen() {
	...
	return (
		...
		>
			...
			<View
				style={[shared.card, { backgroundColor: t.card, marginBottom: 16 }]}
			>
				<View
					style={{
						flexDirection: "row",
				...
				>
					...
					<View style={{ marginLeft: 12, alignItems: "center" }}>
						...
						<View style={{ flexDirection: "row", alignItems: "center" }}>
							...
							/>
							<Text
								style={{ color: t.textSecondary, fontSize: 13, marginLeft: 4 }}
							>
								%
							</Text>
						</View>
...
function EnvelopeTab({
	...
	const envelopeDefaults = useMemo(() => {
		...
		for (const account of portfolio.accounts) {
			...
			valueByEnvelope.set(
				env,
				(valueByEnvelope.get(env) ?? 0) + account.marketValue,
			);
		}
	...
	return (
		<View>
			{properties.map((property) => {
				...
				const projectedValue = lastYear?.propertyValue ?? 0;
				const realValue = lastYear?.realPropertyValue ?? 0;
				const equity = projection.finalEquity;
				const remainingLoan = lastYear?.remainingLoan ?? 0;
				const year1Cf = projection.years[0]?.cashFlowAfterTax ?? 0;

				return (
					<View
						key={property.id}
						style={[shared.card, { backgroundColor: t.card, marginBottom: 12 }]}
					>
						<Text
							style={{
								color: t.text,
						...
						<DataRow
							label="Valeur actuelle (quote-part)"
							value={formatEuro(property.valeurActuelle * property.partDetenue)}
							theme={t}
						/>
						<DataRow
							label="Revalorisation"
							value={`${(property.revaloAnnuelle * 100).toFixed(1)}% / an`}
							theme={t}
						/>
						<DataRow
							label="Cash-flow année 1 (après impôt)"
							value={formatEuro(year1Cf)}
							valueColor={year1Cf > 0 ? t.success : t.danger}
							theme={t}
						/>
						<DataRow
							label={`Cash-flows cumulés (${horizonYears} ans)`}
							value={formatEuro(projection.cumulativeNetCashFlow)}
							valueColor={
								projection.cumulativeNetCashFlow > 0 ? t.success : t.danger
							}
							theme={t}
						/>
						{property.montantEmprunte > 0 && (
							<DataRow
								...
								value={formatEuro(remainingLoan)}
								theme={t}
							/>
						)}
						{projection.cagr != null && (
							<DataRow
								label="CAGR (apport → net si revente)"
								value={`${(projection.cagr * 100).toFixed(1)} %`}
								theme={t}
							/>
						)}
						{projection.irr != null && (
							<DataRow
								label="TRI (flux annuels)"
								value={`${(projection.irr * 100).toFixed(1)} %`}
								theme={t}
							/>
						)}
						<View
							style={[shared.separator, { backgroundColor: t.cardBorder }]}
						/>
						<DataRow
							label={`Patrimoine net estimé à ${horizonYears} ans`}
							value={formatEuro(equity)}
							valueColor={t.success}
							bold
							theme={t}
						/>
...
function EnvelopeCard({
	...
	return (
		<View style={[shared.card, { backgroundColor: t.card, marginBottom: 8 }]}>
			<Text
				style={{
					color: t.text,
					fontSize: 15,
			...
			{extraStreams.length > 0 && (
				...
				>
					{extraStreams.map((stream) => {
						...
						return (
							<View
								key={`${envelope}-extra-${stream.frequency}-${stream.amount}-${stream.paymentMonth ?? 0}`}
								style={{
									borderRadius: 999,
									paddingHorizontal: 8,
									paddingVertical: 3,
			...
			<View style={{ flexDirection: "row", marginBottom: 10 }}>
				...
				<View style={{ flex: 1 }}>
					<Text style={metaLabelStyle(t)}>Projeté ({horizonYears} ans)</Text>
...
function SectionLabel({ text, theme: t }: { text: string; theme: Theme }) {
	return (
		<Text
			style={{
				color: t.textSecondary,
			...
			{tabs.map((item) => (
				<TouchableOpacity
					...
					style={{
						...
						borderRadius: 8,
						alignItems: "center",
						backgroundColor: tab === item.key ? t.accentBg : "transparent",
					}}
				>
					<Text
						style={{
