...
function DcaTab({
	...
	const save = async () => {
		...
		if (!parsed) {
			Alert.alert(
				"Erreur",
				"Vérifie les champs : il faut un libellé, un montant, et au moins une ligne avec un actif et un %.",
			);
			return;
		}
	...
	const deletePlan = (config: DcaConfig) => {
		Alert.alert("Supprimer", `Supprimer le plan "${config.label}" ?`, [
	...
	return (
		<View style={[shared.card, { backgroundColor: t.card }]}>
			<View style={[shared.row, { marginBottom: 8 }]}>
				<Text
					style={{ color: t.text, fontSize: 15, fontWeight: "600", flex: 1 }}
				>
					{config.label}
				</Text>
				<Text style={{ color: t.accent, fontSize: 14, fontWeight: "600" }}>
						...
						{ALL_ENVELOPES.map((env) => (
							<TouchableOpacity
								...
								style={{
									...
									borderRadius: 6,
									backgroundColor:
										draft.envelope === env ? t.accent : t.cardBorder,
								}}
							>
								<Text
									style={{
				...
				{draft.lines.map((line, idx) => (
					...
					>
						<View style={{ flexDirection: "row", gap: 8 }}>
							<View style={{ flex: 2 }}>
								<Text
									style={{ color: t.textMuted, fontSize: 11, marginBottom: 4 }}
								>
									Panier (optionnel)
								</Text>
								<TextInput
						...
						<View style={{ marginTop: 6 }}>
							...
							<TextInput
								...
								placeholder={
									assets.length > 0 ? `Ex: ${assets[0].id}` : "ID1, ID2"
								}
								placeholderTextColor={t.textMuted}
								style={compactInputStyle(t)}
							/>
					...
					>
						...
						>
							...
							<TextInput
								...
								style={{
									...
									backgroundColor: hasOverride ? "#3b82f610" : t.card,
									borderRadius: 8,
									paddingHorizontal: 10,
									paddingVertical: 6,
									fontSize: 14,
						...
						{execution.lines.map((line) => (
							<ExecutionLineRow
								key={line.assetId}
								line={line}
								assetLabel={assetMap.get(line.assetId)?.label ?? line.assetId}
								theme={t}
							/>
						))}
...
function RetraiteTab({ theme: t }: { theme: Theme }) {
	...
	const persist = useCallback(
		async (
			overrides?: Partial<{ birth: string; age: string; pension: string }>,
		) => {
			...
			const profile: Record<string, unknown> = {
				targetRetirementAge: clampRetirementAge(age === "" ? 64 : Number(age)),
			};
...
function PropertyCard({
	...
	const isResidence = property.regime === "RESIDENCE_PRINCIPALE";
			...
			{!isResidence && (
				<>
					<DataRow
						...
						value={`${(snapshot.grossYield * 100).toFixed(1)} %`}
						theme={t}
					/>
					<DataRow
						label="Cash-on-cash après impôt"
						value={`${(snapshot.netYield * 100).toFixed(1)} %`}
						theme={t}
					/>
				</>
			)}
