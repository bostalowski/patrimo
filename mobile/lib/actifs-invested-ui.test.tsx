...
describe("mobile asset invested display", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("shows Investi with cost basis on an open position summary", () => {
    mocks.workbookState = {
      workbook: workbook({
        transactions: [
          {
    ...
    const text = visibleText(render(<ActifsScreen />));
  ...
  it("shows Investi as 0 € when the asset has zero quantity", () => {
    mocks.workbookState = {
      workbook: workbook(),
      prices: new Map([["etf-monde", 120]]),
      loading: false,
