...
4. [docs/howto/feature-flow.md](docs/howto/feature-flow.md) — the canonical cadrage → merge sequence (gates by slug: `branch-contract` … `merge`)
...
make e2e                # verify-e2e: Playwright workbook smoke
...
make red CASE=… CMD=…   # gate red-evidence: write RED only if CMD actually fails (docs/howto/tdd-red-green.md)
...
### Definition of Done (verify bands)

| Slug | Command | When |
|---|---|---|
| `verify-static` | `make verify` | Always before claiming done |
| `verify-behavior` | targeted `npm test -- <path>` | Any behavior change |
| `verify-e2e` | `make e2e` / `make verify-full` | Web UI, `src/app/api`, workbook I/O, settings |

When `verify-behavior` applies: **cadrage lock** first ([docs/howto/cadrage-lock.md](docs/howto/cadrage-lock.md) — Intent, decisions, teach-back, Framer ≠ Maker), then implement each CONTRACT behavior case as **RED → GREEN** (failing targeted test for the missing behavior, then minimal production code). Procedure: [docs/howto/tdd-red-green.md](docs/howto/tdd-red-green.md). Skip cadrage-lock deep checks and RED → GREEN when CONTRACT `verify-behavior` is `n/a` (Tier A).

Do not declare victory on `verify-static` alone when `verify-e2e` applies. One CONTRACT per feature branch — see [docs/agent/branches/README.md](docs/agent/branches/README.md). Lint currently gates `packages/core` + `src` (mobile lint debt is out of gate).
...
| [docs/howto/feature-flow.md](docs/howto/feature-flow.md) | The canonical gate sequence (`branch-contract` … `merge`) from cadrage to merge |
| [docs/howto/](docs/howto/) | Step-by-step procedures (dev setup, release, [cadrage-lock](docs/howto/cadrage-lock.md), [tdd-red-green](docs/howto/tdd-red-green.md), [ui-screenshots-in-pr](docs/howto/ui-screenshots-in-pr.md), implement-*) |
...
| [`.agents/rules/meaningful-step-names.md`](.agents/rules/meaningful-step-names.md) | Always-on rule: gates/DoD bands use meaningful slugs (never bare `G#` / `Layer N`). Canonical under `.agents/rules/`; symlinked for Cursor + Claude. |
...
Full gate-by-gate sequence: [docs/howto/feature-flow.md](docs/howto/feature-flow.md) (`branch-contract` … `merge`). Summary:
...
3. **Work:** implement one tranche at a time. When `verify-behavior` applies: per case **RED → GREEN** ([tdd-red-green.md](docs/howto/tdd-red-green.md)), record RED evidence in branch PROGRESS; update colocated ARCHITECTURE / ADR / glossary with the code.
4. **Verify (`dod-verify`):** DoD bands required above, then `make gauntlet` (test-removal guard + scoped mutation on `@patrimo/core`, workbook I/O, or API-route diffs — CONSTRAINTS §27). When web UI changed: also capture PR screenshots ([ui-screenshots-in-pr.md](docs/howto/ui-screenshots-in-pr.md)). Never claim done on failing verify or gauntlet.
